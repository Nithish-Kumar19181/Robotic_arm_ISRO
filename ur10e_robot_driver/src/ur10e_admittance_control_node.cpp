...
public:
  ...
  {
    RCLCPP_INFO(get_logger(), "Admittance Control Node (JOINT ORDER FIXED)");
  }
...
private:
  ...
  CallbackReturn on_deactivate(const rclcpp_lifecycle::State &) override
  {
    ...
    RCLCPP_WARN(get_logger(), "Admittance control DEACTIVATED");
    return CallbackReturn::SUCCESS;
  }
  ...
  void try_initialize_from_current()
  {
    ...
    if (joint_map_.size() < 6)
      return;

    // EXACT SAME MAPPING AS YOUR WORKING NODES
    q_current_(0) = joint_map_["shoulder_pan_joint"];
    q_current_(1) = joint_map_["shoulder_lift_joint"];
    q_current_(2) = joint_map_["elbow_joint"];
    q_current_(3) = joint_map_["wrist_1_joint"];
    q_current_(4) = joint_map_["wrist_2_joint"];
    q_current_(5) = joint_map_["wrist_3_joint"];
    ...
    RCLCPP_INFO(get_logger(), "Admittance initialized: R=%.3f theta=%.3f",
      radius_, theta_);
