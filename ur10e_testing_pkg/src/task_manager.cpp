...
public:
  TaskManager() : Node("task_manager") {
    ...
    timer_ = create_wall_timer(1s, std::bind(&TaskManager::start, this));
  }
...
private:
  ...
  bool run_a_{true};
  int approach_circle_count_{0};
  int helix_count_{0};
  static constexpr int MAX_APPROACH_CIRCLE_RUNS = 9;
  static constexpr int MAX_HELIX_RUNS = 8;
  ...
  void sendNext() {
    // Stop if approach_circle has already run its max times
    if (run_a_ && approach_circle_count_ >= MAX_APPROACH_CIRCLE_RUNS) {
      RCLCPP_INFO(get_logger(),
                  "approach_circle has completed %d runs. Stopping.",
                  approach_circle_count_);
      return;
    }

    // Stop if helix has already run 8 times
    if (!run_a_ && helix_count_ >= MAX_HELIX_RUNS) {
      RCLCPP_INFO(get_logger(), "helix has completed %d runs. Stopping.",
                  helix_count_);
      return;
    }

    // Increment counter when about to run approach_circle
    if (run_a_) {
      approach_circle_count_++;
      RCLCPP_INFO(get_logger(), "Starting approach_circle run %d / %d",
                  approach_circle_count_, MAX_APPROACH_CIRCLE_RUNS);
    } else {
      helix_count_++;
      RCLCPP_INFO(get_logger(), "Starting helix run %d / %d", helix_count_,
                  MAX_HELIX_RUNS);
    }
    ...
    options.feedback_callback =
        [this](GoalHandleExecuteSkill::SharedPtr,
          ...
          RCLCPP_INFO(get_logger(), "Feedback: %s (%.2f)",
    ...
    options.result_callback =
        [this](const GoalHandleExecuteSkill::WrappedResult &result) {
          if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
            RCLCPP_WARN(get_logger(), "Skill finished: %s",
