...
namespace motion_primitives 
{
    ...
    void CirclePrimitive::onStart(RobotState &robot_state,std::string &urdf_string) 
    {
    if (initialized_) 
    {
        return;
    ...
    void CirclePrimitive::update(RobotState &robot_state, double dt)
    {
        ...
        force_ctrl_.computeAdmittance(measured_force_, target_force_, dt, radius_dot_);
        radius_ += radius_dot_ * dt;
        radius_  = std::clamp(radius_, radius_min_, radius_max_);
    ...
    void CirclePrimitive::targetPose(geometry_msgs::msg::PoseStamped &pose) {
    ...
    void CirclePrimitive::targetWrench(geometry_msgs::msg::WrenchStamped &wrench) {
    ...
    void CirclePrimitive::setParameters(double angular_speed, double max_angle_rad,
         double target_force, double direction) 
    {
    ...
    void CirclePrimitive::setForceGains(double kp, double ki, double integral_limit,double max_rate) 
    {
        force_ctrl_ = force_control::ForceController(kp, ki, integral_limit, max_rate);
    }

    bool CirclePrimitive::isDone() const { return progress_ >= 1.0; }

    double CirclePrimitive::getProgress() const { return progress_; }
