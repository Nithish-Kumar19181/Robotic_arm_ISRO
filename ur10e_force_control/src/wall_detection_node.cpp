...
public:
  ...
  {
    RCLCPP_INFO(get_logger(), "Wall Detection Node (Always initializes from current joints)");
  }
...
private:
  ...
  CallbackReturn on_deactivate(const rclcpp_lifecycle::State &) override
  {
    ...
    RCLCPP_WARN(get_logger(), "Wall detection DEACTIVATED by manager");
    return CallbackReturn::SUCCESS;
  }
  ...
  void on_joint_state(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    // If active but not initialized yet → initialize now
  ...
  void on_urdf(const std_msgs::msg::String::SharedPtr msg)
  {
    ...
    // Try initialize if joint state already received
    try_initialize_from_current();
  }
  ...
  void try_initialize_from_current()
  {
    ...
    for (const auto &segment : chain_.segments)
    {
      ...
      if (joint.getType() != KDL::Joint::None)
      {
        ...
        if (!joint_state_map_.count(name))
          return;
        q_current_(idx++) = joint_state_map_[name];
      }
    }
    ...
    RCLCPP_INFO(get_logger(), "Wall node initialized from current joints at TCP: %.3f %.3f %.3f",
